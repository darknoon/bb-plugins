...
- Each worktree owns a block of 10 consecutive ports, held for as long as the worktree exists.
  Blocks start at 5900 (5900-5909, 5910-5919, …).
- `{port}` is the block's base — use it for the app's dev server. `{port+1}`, `{port+2}` … are the
  ...
  picking a number. An offset past `{port+9}` is an error, not a silent overrun into the next
  worktree.
